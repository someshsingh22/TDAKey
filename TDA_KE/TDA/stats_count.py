...
def adj_m_to_nx_list(adj_matrix, thresholds_array, ntokens, no_mat_output=False):
    ...
    for mat in filt_mat_list:
        nx_graphs_list.append(
            nx.from_numpy_matrix(np.array(mat), create_using=nx.MultiDiGraph())
        )
    if no_mat_output:
...
def adj_ms_to_nx_lists(
    adj_matricies, thresholds_array, ntokens_array, verbose=True, no_mat_output=False
):
    """
...
def edges_f(graph_lists, verbose):
    ...
    iterable = range(len(graph_lists))
    if verbose:
        iterable = tqdm(range(len(graph_lists)), desc="Calc edges amount")
    for i in iterable:
        g_list = graph_lists[i]
        e = []
        for j in range(len(g_list)):
...
def H_1_statistics_by_thresholds(thresholds_array, c_lists, lang_list, layer, head):
    """
    ...
    for l in range(L):
        ...
        for t in range(T):
            ...
            axs[t, l].bar(current_data[l][t], color=current_color)
